...
{
    public class ApproveTrainingSessionAsync
    {
        ...
        public ApproveTrainingSessionAsync()
        {
        ...
        [Fact]
        public async Task ApproveTrainingSessionAsync_ShouldReturnError_WhenSessionNotFound()
        {
            // Arrange
            _trainingSessionRepositoryMock.Setup(x => x.GetTrainingSessionDetailAsync(It.IsAny<string>()))
                .ReturnsAsync((TrainingSession)null);

            var request = new ApproveTrainingSessionRequest { TrainingSessionId = "session123" };

            // Act
            var result = await _service.ApproveTrainingSessionAsync(request);
        ...
        [Fact]
        public async Task ApproveTrainingSessionAsync_ShouldReturnError_WhenSessionNotPending()
        {
            // Arrange
            _trainingSessionRepositoryMock.Setup(x => x.GetTrainingSessionDetailAsync(It.IsAny<string>()))
        ...
        [Fact]
        public async Task ApproveTrainingSessionAsync_ShouldReturnError_WhenUserNotManager()
        {
            // Arrange
            _trainingSessionRepositoryMock.Setup(x => x.GetTrainingSessionDetailAsync(It.IsAny<string>()))
                .ReturnsAsync(new TrainingSession
            ...
            // Assert
            Assert.Null(result.Data);
            Assert.Equal(TrainingSessionMessage.Error.OnlyManagerOfTeamCanAcceptTrainingSession, result.Message);
        }

        [Fact]
        public async Task ApproveTrainingSessionAsync_ShouldReturnSuccess_WhenAllValid()
        {
            // Arrange
            var trainingSession = new TrainingSession
            {
                TrainingSessionId = "session123",
                Status = TrainingSessionConstant.Status.PENDING, // Pending
                TeamId = "team123",
            ...
            _trainingSessionRepositoryMock.Setup(x => x.GetTeamEmailsAsync(It.IsAny<string>()))
                .ReturnsAsync(new List<string> { "team@example.com" });

            _userRepositoryMock.Setup(x => x.GetUserByIdAsync(It.IsAny<string>()))
                .ReturnsAsync(new User { Username = "ManagerUser" });

            var request = new ApproveTrainingSessionRequest
            {
                TrainingSessionId = "session123",
                CourtPrice = 100
            };
            ...
            Assert.Equal(TrainingSessionMessage.Success.TrainingSessionAcceptedSuccessfully, result.Message);
